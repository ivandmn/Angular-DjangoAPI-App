    ...
    try:
        ...
        if not request_data.get('email'):
            ...
            if  request.session.get('reset_password_key') and  request.session.get('reset_password_email') == request_data.get('email'):
                return Response(data={'state':'Password Reset Key as been sent earlier'}, status=status.HTTP_423_LOCKED, template_name='api.api.html')
    ...
    try:
        if not auth.is_authenticated(request):
            ...
            if(default_storage.exists('storage-files/' + file_name)):
                ...
                return Response(data={'deleted': True}, status=status.HTTP_200_OK, template_name='api.api.html')
            else:
                return Response(data={'deleted': False}, status=status.HTTP_200_OK, template_name='api.api.html')
    except Exception as e:
        print("An exception occurred - " + format(e))
        return Response(data={'state':'Server Error'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR, template_name='api.api.html')
