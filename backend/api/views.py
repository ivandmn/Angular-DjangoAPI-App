...
def login(request: HttpRequest):
    try:
        #Check if request payload is json format
        ...
        #Return access_token to client and save in session that user is logged
...
def logout(request: HttpRequest):
    try:
        #If authorization token is not in request headers return error 400 to client
        if not request.headers.get('Authorization'):
            return Response(data={'state':'Authorization failed: auth_token not set'}, status=status.HTTP_403_FORBIDDEN, template_name='api.html') 
        else:
            #Get user access token from request headers
            encoded_token = auth.get_jwt_access_token_from_request_header(request.headers.get('Authorization'))
            #Try to decode token 
            decoded_token = auth.decode_jwt_access_token(encoded_token)
            #If token is invalid, return error 401 to client
            if not decoded_token:
                response = Response(data={'state':'Token not valid'}, status=status.HTTP_401_UNAUTHORIZED, template_name='api.html')
                response.set_cookie('access_token', '', 0)
                return response
            #If token is valid, return success response to client
            else:
...
def validate_email(request: HttpRequest):
    try:
        #Check if request payload is json format
        request_data: dict = request.data
        #If email is not in request return error 401 to client
        if not request_data.get('email'):
            ...
            if  request.session.get('reset_password_key') and  request.session.get('email') == request_data.get('email'):
            ...
            #If reset password key has not been sent earlier or email not equals to session email, enter if
                ...
                #If email exist in db, save email and reset_password_key in session, and send reset_password_key to client enail
                else:
                    #Save email in user session
                    request.session['email'] = email_valid['email']
...
def validate_reset_key(request: HttpRequest):
    try:
        ...
        if not request_data.get('key'):
            ...
            #If session reset_password_key equals request key, set session valid key to true and return success to client
            else:
                request.session['validKey'] = True
                return Response(data={'state': 'Success'}, status=status.HTTP_200_OK, template_name='api.html')
    except Exception as e:
        print("An exception occurred - " + format(e))
        return Response(data={'state':'Server Error'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR, template_name='api.html')
...
def change_user_password(request: HttpRequest):
    try:
        ...
        if not request_data.get('password'):
            ...
            if(request.session.get('validKey') != True and not request.session.get('email')):
                ...
                passwordChanged: bool = utils_db.change_user_password(request.session.get('email'), request_data.get('password'))
